...
--
-- Host: 127.0.0.1
-- Generation Time: Jun 09, 2026 at 07:35 AM
...
INSERT INTO `reports` (`id`, `user_id`, `assigned_to`, `title`, `category`, `description`, `status`, `created_at`, `latitude`, `longitude`, `ai_emotion`, `ai_polarity`, `ai_summary`, `ai_severity`, `ai_polarity_score`, `ai_subjectivity_score`) VALUES
...
(23, NULL, NULL, 'quema', 'Concern', 'Reporte del Incidente: El Quema en el Pueblo\r\n\r\nAyer, un grande quema ya ocurri cerca del oficina y el escuela. El fuego ya empieza accidentalmente porque un persona ya abandona un vela prendido cerca de un cajon de madera. De alisto, el daño ya queda grande; el edificio ya queda machucado y mucho kasangkapan ya queda quemado. El maga gente ya dalih para saca balde con agua para ayuda, mientras el maga vecino ya dale aviso a los bomberos. Gracias a Dios, nadie ya queda muerto, pero el lugar ahora esta abandonado y lleno de uling. Es importante tene paciencia y cuidado para evita este clase de accidente en el futuro.', 'Pending', '2026-05-12 03:47:29', 14.58874355, 120.97739075, 'Sadness', 'Positive', '• incident report: the burning in the town\r\n\r\nYesterday, a large fire occurred near the office and school\n• The fire starts accidentally because a person leaves a lit candle near a wooden box\n• ready, the damage is already great; The building is already damaged and much of the kasangkapan is already burned\n• The people are already there to take out a bucket of water to help, while the neighbor is already alerting the firefighters', 'Critical', 0.172024, 0.479464),
(24, NULL, NULL, 'kema', 'Concern', 'chene kema aki na zamboanga city onde chene ta bende carbon pabor liba bombero para ayuda kunamun aki', 'Pending', '2026-05-12 03:49:29', NULL, NULL, 'Neutral', 'Neutral', '• chain fire aki na zamboanga city where chaine ta binde carbon pabor liba bombero para help kunamun aki', 'High', 0, 0),
(25, NULL, 2, 'temblor', 'Concern', 'IFM FLASH REPORT| Un fuerte temblor ya experiencia el ciudad este lang aga dia de Lunes.\r\nDetalles aqui na 96.3 IFM Zamboanga ', 'Resolved', '2026-06-08 14:15:59', NULL, NULL, 'Fear', 'Positive', '• ifm flash report| The city has already experienced a strong tremor this Monday\n• details here already 96\n• 3 ifm zamboanga', 'Low', 0.616667, 0.816667),
(26, NULL, NULL, 'test 4', 'Complaint', 'Maayong buntag! Karon nga adlaw, nindot kaayo mag-inom og kape samtang naglingkod sa bangko sa gawas. Hilom kaayo ang palibot ug presko ang hangin. Pagkahuman nako og pamahaw, maligo nako ug mag-andam para sa trabaho. Ikaw, kumusta ang imong adlaw karon', 'Ongoing', '2026-06-08 14:18:06', 14.58642024, 120.98531652, 'Joy', 'Positive', '• good morning! today, it is very nice to drink coffee while sitting on the bench outside\n• the environment is very quiet and the air is fresh\n• after I have breakfast, I take a shower and get ready for work\n• you, how is your day today', 'Low', 0.359167, 0.513889),
(27, NULL, 2, 'bisaya', 'Concern', 'Nindot kaayo ang panahon karon. Nagmata ko sayo sa buntag para mopalit ug kape sa merkado. Pag abot nako didto, nakit-an nako ang akong amigo ug nag estorya mi gamay. Human nako ug inom sa kape, nibalik dayon ko sa balay aron magluto sa akong paniudto. Daghang salamat sa Ginoo kay malinawon ug nindot ang akong adlaw', 'Pending', '2026-06-08 14:38:55', 14.59373020, 120.98565986, 'Joy', 'Positive', '• the weather is very nice today\n• I woke up early in the morning to buy coffee at the market\n• when I got there, I saw my friend and we talked a little\n• after drinking coffee, I immediately returned home to cook my lunch', 'Low', 0.342083, 0.593333);
...
INSERT INTO `report_actions` (`id`, `report_id`, `user_id`, `note`, `created_at`) VALUES
(1, 27, 2, 'd', '2026-06-09 04:40:29'),
(2, 27, 1, 'all goods', '2026-06-09 05:08:19'),
(3, 27, 1, 'tst', '2026-06-09 05:25:36'),
(4, 25, 2, 'asda', '2026-06-09 05:34:43'),
(5, 27, 2, 'small changes boss', '2026-06-09 05:34:54');
...
(4, 'joemama', 'joemama@gmail.com', '$2y$10$MY0mV4tcElbxOPrn77i0Fe0jan3gI.ao2m7P0QvdFowxicXSBXRXi', 'resident', '2026-06-08 14:30:42');
...
ALTER TABLE `report_actions`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_report_actions_report_id` (`report_id`),
  ADD KEY `idx_report_actions_user_id` (`user_id`);
...
--
ALTER TABLE `reports`
  ADD CONSTRAINT `fk_reports_assigned_to` FOREIGN KEY (`assigned_to`) REFERENCES `users` (`id`) ON DELETE SET NULL,
  ADD CONSTRAINT `reports_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `report_actions`
--
ALTER TABLE `report_actions`
  ADD CONSTRAINT `fk_report_actions_report` FOREIGN KEY (`report_id`) REFERENCES `reports` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_report_actions_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`) ON DELETE CASCADE;
COMMIT;
